...
const MoviesCardList = ({
  ...
  const filteredMovies = listMovies.slice(0, moviesDisplay).map(movie => {
    return (
      <MoviesCard
        ...
        checkSaved={isSavedMovies ? true : checkSaved(savedFilms, movie)}
  ...
  return (
    <section className="movies-list">
      {listMovies.length !== 0 ? (
        <ul className="movies-list__list">{isSavedMovies ? savedMovies : filteredMovies}</ul>
      ) : (
      ...
      {!isSavedMovies && insertList.length < listMovies.length ? (
        <button type="button" onClick={addCards} className="movies-list__btn-more">
          Ещё
        </button>
      ) : (
