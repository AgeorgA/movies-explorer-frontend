...
const SavedMovies = ({ savedFilms, onDeleteSaveFilm, onBlockedButton }) => {
  ...
  const handleChangeCheckbox = () => {
    if (savedFilms) {
      ...
      if (listMovies !== 0) {
        setListMovies(foundFilms(savedFilms, isRequestBlock, !isCheckedShortFilms));
      } else {
        return;
      }
    }
  ...
  const filtredFilms = request => {
    ...
    setListMovies(foundFilms(savedFilms, request, isCheckedShortFilms));
  };
